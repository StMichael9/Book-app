...
def upgrade() -> None:
    """Upgrade schema."""
    # The original migration assumed these UUID tables had been dropped by
    # hand. Make fresh installs reproducible without silently deleting a
    # populated V1 catalogue.
    connection = op.get_bind()
    for table_name in ("books", "authors", "tags"):
        has_rows = connection.execute(
            sa.text(f"SELECT EXISTS (SELECT 1 FROM {table_name} LIMIT 1)")
        ).scalar_one()
        if has_rows:
            raise RuntimeError(
                "The UUID catalogue contains data. Back it up and migrate it "
                "explicitly before converting to integer primary keys."
            )
